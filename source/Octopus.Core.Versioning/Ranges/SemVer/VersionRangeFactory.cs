// This class is based on NuGet's VersionRange class from https://github.com/NuGetArchive/NuGet.Versioning
...
namespace Octopus.Core.Versioning.Ranges.SemVer
{
    /// <summary>
    ...
    public partial class VersionRange
    {
        ...
        /// </summary>
        public static readonly VersionRange None = new VersionRange(new SemanticVersion(0, 0, 0), false, new SemanticVersion(0, 0, 0), false, false);
        ...
        public static VersionRange Parse(string value, bool allowFloating)
        {
            ...
            if (!TryParse(value, allowFloating, out versionInfo))
            {
                throw new ArgumentException(
                    String.Format(CultureInfo.CurrentCulture, "'{0}' is not a valid version string.", value));
            }
        ...
        public static bool TryParse(string value, bool allowFloating, out VersionRange versionRange)
        {
            ...
            if (!String.IsNullOrWhiteSpace(minVersionString))
            {
                ...
                else
                {
                    ...
                    if (!SemVerFactory.TryCreateVersion(minVersionString, out minVersion))
                    {
                        // invalid version
                        return false;
                    }
            ...
            {
                if (!SemVerFactory.TryCreateVersion(maxVersionString, out maxVersion))
                {
                    // invalid version
                    return false;
                }
        ...
        /// </summary>
        public static VersionRange Combine(IEnumerable<SemanticVersion> versions)
        {
        ...
        /// Returns the smallest range that includes all given versions.
        /// </summary>
        public static VersionRange Combine(IEnumerable<SemanticVersion> versions, IVersionComparer comparer)
        {
            VersionRange result = VersionRange.None;

            if (versions.Any())
            {
                IEnumerable<SemanticVersion> ordered = versions.OrderBy(v => v, comparer);
        ...
        public static VersionRange Combine(IEnumerable<VersionRange> ranges, IVersionComparer comparer)
        {
            ...
            if (ranges.Any())
            {
                ...
                SemanticVersion lowest = first.MinVersion;
                bool includeLowest = first.IsMinInclusive;
                SemanticVersion highest = first.MaxVersion;
