...
namespace Octopus.Versioning.Maven
{
    ...
    public class ComparableVersion : IComparable<ComparableVersion>
    {
        ...
        readonly ListItem items = new ListItem();
            ...
            public int CompareTo(Item? item)
            {
                ...
                switch (item.GetItemType())
                {
                    ...
                    case LIST_ITEM:
                        ...
                        for (var index = 0; index < Math.Max(Count, listItem.Count); ++index)
                        {
                            var left = index < Count ? this[index] : null;
                            var right = index < listItem.Count ? listItem[index] : null;
