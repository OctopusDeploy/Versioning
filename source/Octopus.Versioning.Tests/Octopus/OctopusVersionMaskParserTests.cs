...
namespace Octopus.Versioning.Tests.Octopus
{
    ...
    public class OctopusVersionMaskParserTests
    {
        ...
        [TestCase("c.c.c_i", "whatever", "c.c.c_i", Description = "Not a valid mask (a mask is only valid when the prerelease end with '.i' or '.c'), so the version is passed through.")]
        [TestCase("c.c.c_c", "whatever", "c.c.c_c", Description = "Not a valid mask (a mask is only valid when the prerelease end with '.i' or '.c'), so the version is passed through.")]
        ...
        [TestCase("1.2.3-i", "whatever", "1.2.3-i", Description = "This is not a mask, so keep the original value")]
        [TestCase("1.2.3-c", "whatever", "1.2.3-c", Description = "This is not a mask, so keep the original value")]
